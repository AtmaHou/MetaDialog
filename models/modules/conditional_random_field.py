...
class ConditionalRandomField(torch.nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.num_tags = num_tags  # Accurate num tags, including pad label
        self.no_pad_num_tags = num_tags - 1  # excluding pad label
        self.include_start_end_transitions = include_start_end_transitions
        # _constraint_mask indicates valid transitions (based on supplied constraints).
        # Include special start of sequence (num_tags + 1) and end of sequence tags (num_tags + 2)
        if constraints is None:
            # All transitions are valid.
            constraint_mask = torch.Tensor(self.no_pad_num_tags + 2, self.no_pad_num_tags + 2).fill_(1.)
        else:
            constraint_mask = torch.Tensor(self.no_pad_num_tags + 2, self.no_pad_num_tags + 2).fill_(0.)
    ...
    def _input_likelihood(self,
        ...
        if self.include_start_end_transitions:
            stops = alpha + end_transitions.view(1, num_tags)
        else:
            stops = alpha
    ...
    def _joint_likelihood(self,
        ...
        if self.include_start_end_transitions:
            score = start_transitions.index_select(0, tags[0])
        else:
            score = 0.0
        ...
        if self.include_start_end_transitions:
            last_transition_score = end_transitions.index_select(0, last_tags)
        else:
    ...
    def forward(self,
        ...
        """
        self._constraint_mask = self._constraint_mask.to(inputs.device)
        # pylint: disable=arguments-differ
        if mask is None:
            mask = torch.ones(*tags.size(), dtype=torch.long)

        log_denominator = self._input_likelihood(inputs,
                                                 transitions, start_transitions, end_transitions,
                                                 mask)
        log_numerator = self._joint_likelihood(inputs,
                                               transitions, start_transitions, end_transitions,
                                               tags, mask)
        ...
        if reduction == 'sum':
            ...
            raise ValueError
        # if llh > 0:
        #     print('Debug positive llh loss!!!!!!!', log_numerator, log_denominator)
        #     llh = llh * 0
        return llh

    def viterbi_tags(self,
                     logits: torch.Tensor,
                     transitions_without_constrain: torch.Tensor,
                     start_transitions: torch.Tensor,
                     end_transitions: torch.Tensor,
        ...
        """
        self._constraint_mask = self._constraint_mask.to(logits.device)
        ...
        constrained_transitions = (
                transitions_without_constrain * self._constraint_mask[:num_tags, :num_tags] +
        ...
        if self.include_start_end_transitions:
            transitions[start_tag, :num_tags] = (
                    start_transitions.detach() * self._constraint_mask[start_tag, :num_tags].data +
                    -10000.0 * (1 - self._constraint_mask[start_tag, :num_tags].detach())
            )
            transitions[:num_tags, end_tag] = (
                    end_transitions.detach() * self._constraint_mask[:num_tags, end_tag].data +
